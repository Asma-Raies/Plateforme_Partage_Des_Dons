...
import { NotificationBell } from '../../components/NotificationBell';   // ← Import here
...
const NAV = [
  ...
  {
    to: '/association/campaigns/create',
  ...
  {
    to: '/association/object-donations',
    label: 'Object Donations',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
  ...
  {
    to: '/association/my_notifications',
    label: 'Notifications',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
...
function Avatar({ user, size = 'md' }) {
  ...
  if (user?.avatar) {
    return <img src={user.avatar} alt={user.name} className={`${sizes[size]} rounded-full object-cover ring-2 ring-white`} />;
  }
  return (
    <div className={`${sizes[size]} ${color} rounded-full flex items-center justify-center text-white font-bold ring-2 ring-white`}>
      {initials}
    </div>
  );
}
...
export default function AssociationLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} flex-shrink-0 bg-slate-900 flex flex-col transition-all duration-300 ease-in-out`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-6 flex-shrink-0 shadow-sm">
          <div>
            ...
            <p className="text-xs text-slate-400">Association account</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative" data-profile-menu>
              <button
                ...
                className="flex items-center gap-2.5 pl-1 pr-3 py-1 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-200 transition"
              >
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 py-1">
                  {/* Profile menu content remains the same */}
                  ...
                  <button
                    onClick={() => { navigate('/association/profile'); setShowProfileMenu(false); }}
                  ...
                  <button
                    onClick={() => { navigate('/association/settings'); setShowProfileMenu(false); }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition"
                  >
                    Settings
                  </button>
